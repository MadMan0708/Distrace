...
java -agentpath:"$NATIVE_AGENT_LIB_PATH=instrumentor_server_cp=$INSTRUMETOR_CP;log_dir=$LOG_DIR;log_level=$LOG_LEVEL;instrumentor_lib_jar=$EXAMPLE_SERVER_JAR_PATH;instrumentor_server_jar=$EXAMPLE_SERVER_JAR_PATH;instrumentor_main_class=$INSTRUMENTOR_MAIN_CLASS;connection_str=$CONNECTION_STR" -jar $EXAMPLE_APP_JAR_PATH
