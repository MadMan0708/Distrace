...
using namespace Distrace::Logging;
 ...
 int InstrumentorAPI::send_string_request(std::string data) {
    ...
    std::string InstrumentorAPI::receive_msg_reply() {
        // wait for reply
    ...
    int InstrumentorAPI::send_and_receive(const byte* input_data, int input_data_len, byte** output_buff ){
        ...
        auto length_as_string = receive_msg_reply();
    ...
    bool InstrumentorAPI::should_instrument(std::string class_name){
        ...
        mtx.lock();
        log(LOGGER_INSTRUMENTOR_API)->debug() << "Asking Instrumentor whether it needs to instrument class \"" << class_name << "\"";
        ...
        if(reply == ACK_REQ_INST_YES ){
            log(LOGGER_INSTRUMENTOR_API)->info() << "Instrumentor reply: Class \"" << class_name << "\" will be instrumented.";
            ret_value = true;
        }else if( reply == ACK_REQ_INST_NO){
            log(LOGGER_INSTRUMENTOR_API)->debug() << "Instrumentor reply: Class \"" << class_name << "\" won't be instrumented.";
        }else{
            // never can be here
            assert(false);
        }
    ...
    int InstrumentorAPI::init(std::string path_to_instrumentor_jar) {
        ...
        const std::string addr = Agent::get_arg_value(Agent::ARG_SOCKET_ADDRESS);
        int result = system(Utils::stringToCharPointer("java -jar " + path_to_instrumentor_jar + " "+addr+" & "));
        if(result < 0){
        ...
        if(endpoint < 0){
            log(LOGGER_INSTRUMENTOR_API)->error() << "Returned error code "<< errno << ". Connection to the instrumentor JVM can't be established! Is instrumentor JVM running ?";
            return JNI_ERR;
        }else{
        ...
        std::string instrumentor_jar = Agent::get_arg_value(Agent::ARG_INSTRUMENTOR_JAR);
        ...
        return AgentUtils::check_jvmti_error(Agent::globalData->jvmti, error,
                                      "Path: "+instrumentor_jar+" successfully added on the system's classloader search path",
                                      "Cannot add path "+instrumentor_jar+" on the system's classloader search path");
